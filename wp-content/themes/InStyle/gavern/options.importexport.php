...
?>
	<div class="gkWrap wrap">
		<h1><big><?php echo $gk_tpl->full_name; ?></big><small><?php _e('Based on the Gavern WP framework', GKTPLNAME); ?></small></h1>
		<div class="gkImport">
...
<?php 
		...
		$option_prefix = $gk_tpl->name;
		$blogname = str_replace(" ", "", get_option('blogname'));
		$date = date("d_m_Y_H_i_s");
		$json_name = $blogname."_".$gk_tpl->name."_".$date; // Generating filename.
		...
		if ($options) {
			foreach ($options as $key) {
				if(
					$key->option_name != $gk_tpl->name . '_widget_responsive' &&
					$key->option_name != $gk_tpl->name . '_widget_rules' &&
					$key->option_name != $gk_tpl->name . '_widget_rules_type' &&
					$key->option_name != $gk_tpl->name . '_widget_style' && 
					$key->option_name != $gk_tpl->name . '_widget_users'
				) {
