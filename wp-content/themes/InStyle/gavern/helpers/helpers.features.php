...
function add_gavern_metaboxes() {
	...
	if(get_option($gk_tpl->name . '_seo_use_gk_seo_settings') == 'Y' && get_option($gk_tpl->name . '_seo_post_desc') == 'custom') {
...
function gavern_add_featured_video() {
    global $gk_tpl;
    if(get_option($gk_tpl->name . "_featured_video", 'Y') == 'Y') {
...
function gavern_widget_update($instance, $new_instance, $old_instance, $widget) {	
	...
	if ( isset( $_POST[$gk_tpl->name . '_widget_rules_' . $widget->id] ) ) {	
		...
		$options_type[$widget->id] = $_POST[$gk_tpl->name . '_widget_rules_type_' . $widget->id];
		$options[$widget->id] = $_POST[$gk_tpl->name . '_widget_rules_' . $widget->id];
		$styles[$widget->id] = $_POST[$gk_tpl->name . '_widget_style_' . $widget->id];
		$styles_css[$widget->id] = $_POST[$gk_tpl->name . '_widget_style_css_' . $widget->id];
		$responsive[$widget->id] = $_POST[$gk_tpl->name . '_widget_responsive_' . $widget->id];
		$users[$widget->id] = $_POST[$gk_tpl->name . '_widget_users_' . $widget->id];
		...
		// update the settings
		update_option($gk_tpl->name . '_widget_rules_type', $options_type);
		update_option($gk_tpl->name . '_widget_rules', $options);
		update_option($gk_tpl->name . '_widget_style', $styles);
		update_option($gk_tpl->name . '_widget_style_css', $styles_css);
		update_option($gk_tpl->name . '_widget_responsive', $responsive);
		update_option($gk_tpl->name . '_widget_users', $users);
	}	
...
function gavern_widget_control() {	
	...
	if(get_option($gk_tpl->name . '_widget_rules_state') == 'Y') {
		...
		<div class="gk_widget_rules_wrapper'.((isset($_COOKIE['gk_last_opened_widget_rules_wrap']) && $_COOKIE['gk_last_opened_widget_rules_wrap'] == 'gk_widget_rules_form_'.$id) ? ' active' : '').'" data-id="gk_widget_rules_form_'.$id.'">
			<p>
				<label for="' . $gk_tpl->name . '_widget_rules_'.$id.'">'.__('Visible at: ', GKTPLNAME).'</label>
				<select name="' . $gk_tpl->name . '_widget_rules_type_'.$id.'" id="' . $gk_tpl->name . '_widget_rules_type_'.$id.'" class="gk_widget_rules_select">
			...
			<fieldset class="gk_widget_rules_form" id="gk_widget_rules_form_'.$unique_id.'" data-id="gk_widget_rules_form_'.$id.'">
				<legend>'.__('Select page to add', GKTPLNAME).'</legend>
				 <select class="gk_widget_rules_form_select">
				 	...
				 	<option value="product_cat:">'.__('Product category', GKTPLNAME).'</option>
				 	<option value="shop">'.__('Shop', GKTPLNAME).'</option>
				 </select>
				 ...
				 <p><label>'.__('Product category:', GKTPLNAME).'<input type="text" class="gk_widget_rules_form_input_product_cat" /></label></p>
				 <p><button class="gk_widget_rules_btn button-secondary">'.__('Add page', GKTPLNAME).'</button></p>
				 <input type="text" name="' . $gk_tpl->name . '_widget_rules_'.$id.'"  id="' . $gk_tpl->name . '_widget_rules_'.$id.'" value="'.$value.'" class="gk_widget_rules_output" />
...
function gavern_add_widget_control() {	
	...
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {	
		foreach ( (array) $_POST['widget-id'] as $widget_number => $widget_id ) {
			...
			if (isset($_POST[$gk_tpl->name . '_widget_rules_type_' . $widget_id])) {
				$options_type[$widget_id] = $_POST[$gk_tpl->name . '_widget_rules_type_' . $widget_id];
			}
			// save widget rules
			if (isset($_POST[$gk_tpl->name . '_widget_rules_' . $widget_id])) {
				$options[$widget_id] = $_POST[$gk_tpl->name . '_widget_rules_' . $widget_id];
			}
			// save widget style
			if (isset($_POST[$gk_tpl->name . '_widget_style_' . $widget_id])) {
				$styles[$widget_id] = $_POST[$gk_tpl->name . '_widget_style_' . $widget_id];
			}
			// save widget style CSS
			if (isset($_POST[$gk_tpl->name . '_widget_style_css_' . $widget_id])) {
				$styles_css[$widget_id] = $_POST[$gk_tpl->name . '_widget_style_css_' . $widget_id];
			}
			// save widget responsive
			if (isset($_POST[$gk_tpl->name . '_widget_responsive_' . $widget_id])) {
				$responsive[$widget_id] = $_POST[$gk_tpl->name . '_widget_responsive_' . $widget_id];
			}
			// save widget users
			if (isset($_POST[$gk_tpl->name . '_widget_users_' . $widget_id])) {
				$users[$widget_id] = $_POST[$gk_tpl->name . '_widget_users_' . $widget_id];
			}
		}
	}
	// save the widget id
...
function gavern_widget_control_styles_list($widget_name, $id, $value1, $value2, $value3, $value4 = '') {
	...
	$json_data = $gk_tpl->get_json('config','widgets.styles');
	...
	// output the control
	echo '<div>';
	echo '<p><label for="' . $gk_tpl->name . '_widget_style_'.$id.'">'.__('Widget style: ', GKTPLNAME).'<select name="' . $gk_tpl->name . '_widget_style_'.$id.'"  id="' . $gk_tpl->name . '_widget_style_'.$id.'" class="gk_widget_rules_select_styles">';
	foreach($items as $item) echo $item;
	echo '</select></label></p>';
	//
	echo '<p'.(($value1 != 'gkcustom') ? ' class="gk-unvisible"' : '').'><label for="' . $gk_tpl->name . '_widget_style_css_'.$id.'">'.__('Custom CSS class: ', GKTPLNAME).'<input type="text" name="' . $gk_tpl->name . '_widget_style_css_'.$id.'"  id="' . $gk_tpl->name . '_widget_style_class_'.$id.'" value="'.$value4.'" /></label></p>';
	...
	//
	echo '<p><label for="' . $gk_tpl->name . '_widget_responsive_'.$id.'">'.__('Visible on: ', GKTPLNAME).'<select name="' . $gk_tpl->name . '_widget_responsive_'.$id.'"  id="' . $gk_tpl->name . '_widget_responsive_'.$id.'">';
	//
	...
	//
	echo '<p><label for="' . $gk_tpl->name . '_widget_users_'.$id.'">'.__('Visible for: ', GKTPLNAME).'<select name="' . $gk_tpl->name . '_widget_users_'.$id.'"  id="' . $gk_tpl->name . '_widget_users_'.$id.'">';
	//
	foreach($items as $item) {
		echo $item;
	}
...
function gavern_show_og_meta_box() {
	...
	$custom_meta_fields = $gk_tpl->get_json('config', 'opengraph');
	...
	foreach ($custom_meta_fields as $field) {
		...
		echo '<tr>
				...
				switch($field->type) {
					...
					case 'image':
						$image = 'none';
						if (get_option($gk_tpl->name . '_og_default_image', '') != '')  {
							$image = get_option($gk_tpl->name . '_og_default_image'); 
						}
...
function gavern_show_twitter_meta_box() {
	global $gk_tpl, $post;
	// load custom meta fields
	$custom_meta_fields = $gk_tpl->get_json('config', 'twitter');
	// Use nonce for verification
...
if(get_option($gk_tpl->name . "_opensearch_use_opensearch", "Y") == "Y") {
...
if(get_option($gk_tpl->name . "_shortcodes_widget_state", "Y") == "Y") {
	add_filter('widget_text', 'do_shortcode');
}
	
if(get_option($gk_tpl->name . "_emoticons_widget_state", "Y") == "Y") {
