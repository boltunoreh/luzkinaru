...
function gavern_menu($menuname, $fullname, $params = null) {			
	...
	if(gk_show_menu($menuname)) {
		...
		wp_nav_menu(array(
			  ...
			  'menu_class'      => 'menu ' . $gk_tpl->menu[$menuname]['style'] . ' ' . ((get_option($gk_tpl->name . '_menu_type', 'overlay') == 'overlay') ? 'gk-menu-overlay' : 'gk-menu-classic'), 
			  ...
			  'depth'           => $gk_tpl->menu[$menuname]['depth'],
...
function gavern_comment_template( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>">	
			<aside>
				<?php echo get_avatar( $comment, ($comment->comment_parent == '0') ? 60 : 48); ?>
			</aside>
...
function gk_post_fields() {
	...
	if(get_option($gk_tpl->name . '_custom_fields_state', 'Y') == 'Y') {
		// get the post custom fields
		if ($keys = get_post_custom_keys()) {
			// get the hidden fields array
			$hiddenfields = explode(',', get_option($gk_tpl->name . '_hidden_post_fields', ''));
			...
			foreach ((array) $keys as $key) {
				...
				$mapping = preg_split('/\r\n|\r|\n/', get_option($gk_tpl->name . '_post_fileds_label_mapping'));
 	...
 	?> 
	 	...
	 	<ul>
	 		...
	 		<?php if(!(is_tag() || is_search())) : ?>
	 			...
	 			<?php if((is_tag() || is_search()) || (get_post_type() != 'page' && $param_category) && !is_attachment() ) : ?>
	 			<li class="category">
	 		...
	 		<li class="bookmark">
	 			...
	 			<?php echo edit_post_link(__( 'Edit', GKTPLNAME ), ' ', ''); ?>
	 		</li>
	 		<?php endif; ?>
	 	</ul>
 		<?php endif; ?>
...
function gk_social_api($title, $postID) {
	...
	if($social_api_mode != 'none' && $is_excluded) {
		...
		$pinterest_output = '';
		// FB like
		if(get_option($gk_tpl->name . '_fb_like', 'Y') == 'Y') {
			// configure FB like
			$fb_like_attributes = ''; 
			// configure FB like
			if(get_option($gk_tpl->name . '_fb_like_send', 'Y') == 'Y') { $fb_like_attributes .= ' data-send="true"'; }
			$fb_like_attributes .= ' data-layout="'.get_option($gk_tpl->name . '_fb_like_layout', 'standard').'"';
			$fb_like_attributes .= ' data-show-faces="'.get_option($gk_tpl->name . '_fb_like_show_faces', 'true').'"';
			$fb_like_attributes .= ' data-width="'.get_option($gk_tpl->name . '_fb_like_width', '500').'"';
			$fb_like_attributes .= ' data-action="'.get_option($gk_tpl->name . '_fb_like_action', 'like').'"';
			$fb_like_attributes .= ' data-font="'.get_option($gk_tpl->name . '_fb_like_font', 'arial').'"';
			$fb_like_attributes .= ' data-colorscheme="'.get_option($gk_tpl->name . '_fb_like_colorscheme', 'light').'"';
			
			$fb_like_output = '<div class="fb-like" data-href="'.get_permalink($postID).'" '.$fb_like_attributes.'></div>';
		}
		// G+
		if(get_option($gk_tpl->name . '_google_plus', 'Y') == 'Y') {
			// configure +1 button
			$gplus_attributes = '';    		
			// configure +1 button attributes
			$gplus_attributes .= ' annotation="'.get_option($gk_tpl->name . '_google_plus_count', 'none').'"'; 
			$gplus_attributes .= ' width="'.get_option($gk_tpl->name . '_google_plus_width', '300').'"'; 
			$gplus_attributes .= ' expandTo="top"'; 
				
			if(get_option($gk_tpl->name . '_google_plus_size', 'medium') != 'standard') { 
				$gplus_attributes .= ' size="'.get_option($gk_tpl->name . '_google_plus_size', 'medium').'"'; 
			}
			
			$gplus_output = '<g:plusone '.$gplus_attributes.' callback="'.get_permalink($postID).'"></g:plusone>';
		}
		// Twitter
		if(get_option($gk_tpl->name . '_tweet_btn', 'Y') == 'Y') {
			// configure Twitter buttons    		  
			$tweet_btn_attributes = '';
			$tweet_btn_attributes .= ' data-count="'.get_option($gk_tpl->name . '_tweet_btn_data_count', 'vertical').'"';
			if(get_option($gk_tpl->name . '_tweet_btn_data_via', '') != '') {
				$tweet_btn_attributes .= ' data-via="'.get_option($gk_tpl->name . '_tweet_btn_data_via', '').'"'; 
			}
			$tweet_btn_attributes .= ' data-lang="'.get_option($gk_tpl->name . '_tweet_btn_data_lang', 'en').'"';
		...
		// Pinterest
		if(get_option($gk_tpl->name . '_pinterest_btn', 'Y') == 'Y') {
		     ...
		     if($image == '') {
			     ...
			     if($image == '' && get_option($gk_tpl->name . '_og_default_image', '') != '') {
			          	$image = get_option($gk_tpl->name . '_og_default_image', '');
			     }
		     
		     }
		     // configure Pinterest buttons               
		     $pinterest_btn_attributes = get_option($gk_tpl->name . '_pinterest_btn_style', 'horizontal');
...
function gk_author($author_page = false, $return_value = false) {
	...
	$authorinfo_mode = get_option($gk_tpl->name . '_authorinfo_exclude_include', 'exclude');
	$authorinfo_articles = explode(',', get_option($gk_tpl->name . '_authorinfo_articles', ''));
	$authorinfo_pages = explode(',', get_option($gk_tpl->name . '_authorinfo_pages', ''));
	$authorinfo_categories = explode(',', get_option($gk_tpl->name . '_authorinfo_categories', ''));
	//
	$is_excluded = false;
	//
	...
	if($authorinfo_mode != 'none' && $is_excluded) :
		if(
			(is_page() && get_option($gk_tpl->name . '_template_show_author_info_on_pages') == 'Y') ||
			!is_page()
		) :
		    if(
		        ...
		        (
		        	$author_page ||
		        	get_option($gk_tpl->name . '_template_show_author_info') == 'Y'
		        )
		    ): 
		    ?>
