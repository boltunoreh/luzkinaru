 ...
 *
 * @return the updated array of the contact methods
 *
 **/
...
/**
 *
 * Code used to implement small elements in the widget titles for bigtitle and mediumtitle styles
 *
 * @return null
 * 
 **/
function gavern_widget_title_syntax($title) {
	if($title == '&nbsp;' || trim($title) == '' || strlen($title) == 0) {
		return false;
	} else {
		//
...
add_filter('widget_title', 'gavern_widget_title_syntax');

/**
 *
 * Code used to implement Font Awesome icons in the widget titles
 *
 * @return null
 * 
 **/
function gavern_widget_title_icon($title) {
	if($title == '&nbsp;' || trim($title) == '' || strlen($title) == 0) {
		return false;
	} else {
		//
		$title = preg_replace('@\[(.*?)\]@', '<i class = $1></i>', $title);
		//
		return $title;
	}
}

add_filter('widget_title', 'gavern_widget_title_icon');
...
function gavern_post_aside_class($classes) {
	...
	if(get_option($gk_tpl->name . '_post_aside_state', 'Y') == 'N' || !$param_aside) {
...
if(get_option($gk_tpl->name . '_comments_autoblank', 'Y') == 'Y') {
...
if(get_option($gk_tpl->name . '_comments_autolinking', 'Y') == 'N') {
