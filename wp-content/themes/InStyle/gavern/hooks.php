...
function gavernwp_html_attributes_hook() {
	...
	if(trim(get_option($gk_tpl->name . '_cache_manifest', '')) != '') {
		echo ' manifest="'.trim(get_option($gk_tpl->name . '_cache_manifest', '')).'"';
	}
 	// YOUR HOOK CODE HERE
}
...
function gavernwp_body_attributes_hook() {
 	...
 	if(is_home()) {
 		$classes = '';
 		if($gk_tpl->browser->get("browser") == 'ie8') {
 			$classes .= ' ie8mode';
 		} 
 		
 		if(get_option($gk_tpl->name . '_inner_inset_position', 'right') == 'left') {
 			$classes .= ' gk-inset-left';
 		}
 				
 		body_class('frontpage' . $classes);
 	} else {
 		$classes = '';
 	...
 	if($gk_tpl->browser->get("tablet") == true) {
 		echo ' data-tablet="true"';
 	} 
 	// generate the mobile attribute
 	if($gk_tpl->browser->get("mobile") == true) {
 	    echo ' data-mobile="true"';
 	} 
 	// generate the table-width attribute
 	echo ' data-tablet-width="'. get_option($gk_tpl->name . '_tablet_width', 800) .'"';
...
function gavernwp_ga_code_hook() {
	...
	if(get_option($gk_tpl->name . '_ga_ua_id', '') != '') {
		?>
		<script type="text/javascript">
		  var _gaq = _gaq || [];
		  _gaq.push(['_setAccount', '<?php echo get_option($gk_tpl->name . '_ga_ua_id', ''); ?>']);
...
function gavernwp_custom_rss_feed_url( $output, $feed ) {
    global $gk_tpl;
    // get the new RSS URL
    $feed_link = get_option($gk_tpl->name . '_custom_rss_feed', '');
    // check the URL
    if(trim($feed_link) !== '') {
